...
export default function App() {
  ...
  const searchMovies = (str, type = "all") => {
    loadingTimeout();
    fetch(
      `https://www.omdbapi.com/?apikey=bc5839e5&s=${
        str.length ? str : "panda"
      }${type !== "all" ? `&type=${type}` : ""}`
    )
      .then((response) => response.json())
      .then((data) => {
  ...
  return (
    <>
      ...
      />
      <main>
        <Aside toggleSidebar={toggleSidebar} loadingTimeout={loadingTimeout} />
        <Routes>
          <Route
            exact
            path="/"
      ...
      <Footer loadingTimeout={loadingTimeout} />
    </>
  );
}
